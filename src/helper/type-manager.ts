...
export class TypeMap {
  ...
  lookupTypeOfNamespace(item: ASTBase): TypeInfo | null {
    const me = this;
    const namespace = removeContextPrefixInNamespace(
      transformASTToNamespace(item)
    );
      ...
      switch (origin.type) {
        ...
        case ASTType.Identifier: {
          ...
          if (currentMetaInfo instanceof TypeInfoWithDefinition) {
            ...
            definitions = miniscriptMeta.getDefinitions(definition.returns);
          } else {
            definitions = miniscriptMeta.getDefinitions(currentMetaInfo.type);
          }
  ...
  private resolveIdentifier(item: ASTIdentifier): TypeInfo | null {
    ...
    const defaultDef = miniscriptMeta.getDefinition(['general'], name);
